...
const EditarUsuario = () => {
    ...
    const [newUsuario, setNewUsuario] = useState({ nome: '', sobrenome: '', email: '', cargo: '', planta: '', acessoKPI: false, admin: false, foto: '', login: '', senha: '' });
    const [errorMessage, setErrorMessage] = useState("");

    useEffect(() => {
        axios.get(`http://localhost:5555/Usuarios/${id}`)
            .then((response) => {
    ...
    const handleFormSubmit = (event) => {
        ...
        if (!newUsuario.nome || !newUsuario.sobrenome || !newUsuario.email || !newUsuario.cargo || !newUsuario.planta || !newUsuario.login || !newUsuario.senha || !newUsuario.foto) {
            setErrorMessage("Preencha todos os campos");
            return;
        }

        axios
            .put(`http://localhost:5555/Usuarios/${id}`, newUsuario)
            .then((response) => {
    ...
    return (
        <form className="usuario-app-form" onSubmit={handleFormSubmit}>
            ...
            <div className='usuario-oiii'>
                <h2 className='usuario-texto'>Editar dados do usuário</h2>
                <label className='usuario-texto'>Foto:</label>
                <div className="usuario-image-container">
                    {image === "" || image === null ? "" : <img width={100} height={100} src={image} alt="User" />}
                </div>
